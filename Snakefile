...
TOPGROUP = "rhesus chimp human gorilla".split()
...
rule faTofq :
     ...
     shell  : "cd ice_clustering/{wildcards.top} ; ln -s ../../{input.READS} {wildcards.top}_{wildcards.type}.fasta ; ~/projects/cDNA_Cupcake/sequence/fa2fq.py {wildcards.top}_{wildcards.type}.fasta"

rule catfa  :
     input  : expand("primer_trimmed/{{top}}/{names}/isoseq_{{type}}.fasta", names=NAMES)
...
rule trim   :
     ...
     output : "primer_trimmed/{top}/{names}/isoseq_{type}.fasta"
     params :  sge_opts="-l mfree=20G -l h_rt=4:00:00 -q eichler-short.q -V -cwd"
     shell  : "cd primer_trimmed/{wildcards.top}/{wildcards.names} ; rm -rf * ; ln -s ../../../{input.FAS} ccs.fasta ; cp ../../../{input.BCC} . ; source {input.BCC}"
